...
* ZBS DAL >> Contacts
*
* @author   Woody Hayday <hello@zerobscrm.com>
* @version  2.0
* @access   public
* @see      https://zerobscrm.com/kb
*/
class zbsDAL_contacts extends zbsDAL_ObjectLayer {
        ...
        if (
            ...
            #} ============= PRE-QUERY ============
                ...
                if ($withValues && !$onlyID){
                    ...
                    // as there's a setting, we only include transactions with statuses logged in setting
                    $transStatuses = $this->DAL()->transactions->getTransactionStatusesToInclude(); 

                        // if any, create a status addition to query:
                        // we spot-prep this, due to: https://stackoverflow.com/questions/10634058/issue-when-trying-to-use-in-in-wpdb
                        /*$transStatusQueryAdd = ''; if (is_array($transStatuses) && count($transStatuses) > 0){

                            $transStatusQueryAdd .= ' AND zbst_status IN (%s)';
                            $transStatusesStr = '';
                            foreach ($transStatuses as $ts){
                                if (!empty($ts)){
                                    if ($transStatusesStr !== '') $transStatusesStr .= ',';
                                    $transStatusesStr .= "'".$ts."'";
                                }

                            }
                            // add param, twice, (used below too to calc total paid)
                            $params[] = $transStatusesStr;
                            $params[] = $transStatusesStr;
                        } */                    
                        $transStatusQueryAdd = ''; 
                        if (is_array($transStatuses) && count($transStatuses) > 0){

                            $transStatusesStr = array_map(function($v) {
                                return "'" . esc_sql($v) . "'";
                            }, $transStatuses);
                            $transStatusesStr = implode(',', $transStatusesStr);

                            $transStatusQueryAdd .= ' AND zbst_status IN ('.$transStatusesStr.')';

                        }

                        // transtotal.zbst_status IN (%s) 
    ...
    public function getContacts($args=array()){
        ...
        #} ============= PRE-QUERY ============
            ...
            // Add any addr custom fields for addr1+addr2
            // no need if simpliefied or count
        ...
        #} ============= WHERE ================
            ...
            if (!empty($searchPhrase)){
                ...
                if ($b2bMode) $searchWheres['incompanywithname'] = array('ID','IN','(SELECT DISTINCT zbsol_objid_from FROM '.$ZBSCRM_t['objlinks']." WHERE zbsol_objtype_from = ".ZBS_TYPE_CONTACT." AND zbsol_objtype_to = ".ZBS_TYPE_COMPANY." AND zbsol_objid_to IN (SELECT ID FROM ".$ZBSCRM_t['companies']." WHERE zbsco_name LIKE %s))",'%'.$searchPhrase.'%');
    ...
    private function tidy_contact($obj=false,$withCustomFields=false){
        ...
        global $zbs;
            ...
            */
            ...
            $res['created_date'] = (isset($obj->zbsc_created) && $obj->zbsc_created > 0) ? zeroBSCRM_date_i18n(-1,$obj->zbsc_created,false,true) : false;
            $res['lastupdated'] = $obj->zbsc_lastupdated;
            $res['lastupdated_date'] = (isset($obj->zbsc_lastupdated) && $obj->zbsc_lastupdated > 0) ? zeroBSCRM_locale_utsToDatetime($obj->zbsc_lastupdated) : false;
            $res['lastcontacteduts'] = $obj->zbsc_lastcontacted; // this is the UTS (int14)
            $res['lastupdated_date'] = (isset($obj->zbsc_lastcontacted) && $obj->zbsc_lastcontacted > 0) ? zeroBSCRM_locale_utsToDatetime($obj->zbsc_lastcontacted) : false;
        ...
        if (is_array($contact) && isset($contact['id'])){
            ...
            #} Custom columns
                ...
                if (in_array('quotetotal', $columnsRequired)){
                    ...
                    $resArr['quotestotal'] = zeroBSCRM_formatCurrency(0); if (isset($contact['quotes_value'])) $resArr['quotestotal'] = zeroBSCRM_formatCurrency($contact['quotes_value']);
                ...
                if (in_array('invoicetotal', $columnsRequired)){
                    ...
                    $resArr['invoicestotal'] = zeroBSCRM_formatCurrency(0); if (isset($contact['invoices_value'])) $resArr['invoicestotal'] = zeroBSCRM_formatCurrency($contact['invoices_value']);
                ...
                //if (in_array('transactioncount', $columnsRequired)){
                 ...
                 //   $resArr['transactions'] = $contact['transactions'];
