...
function zeroBSCRM_export_tools_header(){
	...
	if($b2bMode){
		...
		$export_items = array('Companies', 'Contacts', 'Invoices','Quotes','Transactions');
	}else{
		$zbs_cust = 'Export Contacts';
...
function zeroBSCRM_pages_export_tools(){ ?>
	...
	if(isset($_GET['zbswhat']) && $_GET['zbswhat'] == 'companies'){
		...
		if($b2bMode){
			...
			echo "<p>";
			_e("Choose which fields you want to export. Total Companies in database: ","zerbscrm"); 
			echo "<b>" . $co . "</b>";
			echo "</p>";
	    ...
	    //    zbs_prettyprint($fields);
			...
			echo '<input type="checkbox" name="zbs_export_group[]" value="id" /> ID<br />';
		 		...
		 		?>
				<br/>
		       <input type="submit" name="download_co_csv" class="button-primary" value="<?php _e('Export Companies'); ?>" />
...
function zbs_export_invoices(){
	...
	if ( 
		...
		if(zeroBSCRM_permsInvoices()){
			...
			header('Content-Disposition: attachment; filename= ' . $filename);
			$output = fopen('php://output', 'w');
			fputcsv($output, array('Number','Status', 'Customer Name','Company Name', 'Value', 'Created'));		
