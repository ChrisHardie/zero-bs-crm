...
class zeroBSCRM_Plugin_Updater {
		...
		if($installedExts > 0 || $this->get_license_key() !== false){
			...
			// if got response
		  	if ( ! is_wp_error( $response )) {
					...
					if (is_array($official_ver)) foreach ($official_ver as $extUpdateInfo){
						...
						if (isset($extUpdateInfo['zbskey']) && $extUpdateInfo['zbskey'] == $pluginDetails['key']){
							...
							if (version_compare( $pluginDetails['ver'], $extUpdateInfo['version'], '<' )){
								...
								// generate nag
								// then we are adding the notification to "please update Zero BS CRM extension "X")
				...
				// If not all the Zero BS CRM extensions are updated. Fire off the warning shot.
	...
	public function get_info($obj, $action, $arg){
		...
		if ($action=='plugin_information' && isset($arg->slug)){
			...
			// or is it core?
			if (!is_array($possibleExtension)) $possibleExtension = $this->getCoreWLCRM($arg->slug);
			...
			if (is_array($possibleExtension)){
				...
				// do api request to get the info from api.zerobscrm.com
	...
	/* not used
	public function isCRMCoreWL($slug=''){
	...
	public function api_request($action, $data){
		...
		*/
		...
		if ($action !== 'ext_info' && isset($zbsExtUpdateCache) && is_array($zbsExtUpdateCache)){
			...
			// https://epicplugins.groovehq.com/groove_client/mailboxes/49227/filters/188769/tickets/104614323
...
/*
	...
	This function connects to https://api.zerobscrm.com/localcheck
