...
function zeroBSCRM_getLog($lID=-1){
	...
	if ($lID !== -1){
		...
		$retObj = array(
						...
						// Added as per Troy diff file: https://epicplugins.groovehq.com/groove_client/mailboxes/49227/filters/188773/tickets/76216446
...
function zeroBS_getAssigneeMobile($cID =-1){
   ...
   // if returnAsInt - will return -1 for not published, -2 for not accepted, or 14int timestamp for accepted
	...
	function zeroBSCRM_getContactOrCustomer(){
	    ...
	    $theStr = __('Contact',"zero-bs-crm"); if ($b2bMode) $theStr = __('Contact',"zero-bs-crm");
...
function zeroBS_getCompany($coID=-1,$withTransactions=false){
	...
	if ($coID !== -1){
		...
		$retObj = array(
						...
						// e.g. April 27, 2018
						// and we need smt str-to-time will accept (re: https://epicplugins.groovehq.com/groove_client/mailboxes/49227/filters/188769/tickets/77656918)
...
function zeroBS_getCustomerExtrasViaSQL($customerID=-1){
	...
	// this is a temp fix to stop weird dupes (https://zbsltd.atlassian.net/browse/ZBS-827)
...
function zeroBSCRM_clearCPTAutoDrafts(){
			...
			foreach ($zbsCustomPostTypes as $cpt){
				...
				# don't use ! update_option('db_last_auto_cnl',time());
...
function zeroBSCRM_getAddressCustomFields(){
    ...
    // wh added triple checks for some weird bug in client portal? https://epicplugins.groovehq.com/groove_client/mailboxes/49227/filters/188769/tickets/72346671
