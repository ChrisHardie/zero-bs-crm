...
class zbsDAL_quotes extends zbsDAL_ObjectLayer {
	...
	protected $objectModel = array(
        ...
        'send_attachments' => array('fieldname' => 'zbsq_send_attachments', 'format' => 'bool'),
			...
			#} ============= PRE-QUERY ============
				...
				if ($withCustomFields && !$onlyID){
					...
					if (is_array($custFields)) foreach ($custFields as $cK => $cF){
						...
						// add as subquery
						$extraSelect .= ',(SELECT zbscf_objval FROM '.$ZBSCRM_t['customfields']." WHERE zbscf_objid = quote.ID AND zbscf_objkey = %s AND zbscf_objtype = %d) '".$cK."'";
	...
	public function getQuotes($args=array()){
		...
		#} ============= PRE-QUERY ============
			...
			if ($withCustomFields){
				...
				if (is_array($custFields)) foreach ($custFields as $cK => $cF){
					...
					// add as subquery
					$extraSelect .= ',(SELECT zbscf_objval FROM '.$ZBSCRM_t['customfields']." WHERE zbscf_objid = quote.ID AND zbscf_objkey = %s AND zbscf_objtype = %d) ".$cKey;
		...
		#} ============= WHERE ================
			...
			if (!empty($searchPhrase)){
				...
				$searchWheres['search_id2'] = array('id_override','LIKE','%s','%'.$searchPhrase.'%');
