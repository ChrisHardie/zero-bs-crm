   		...
   		<div class='container'>
   				<div class='row customer-list' style="margin-right:70px;">
   					<div class='col-md-8 header-pad'>
   						<h4 class='customer-header'><?php _e('Export ' . zeroBSCRM_getContactOrCustomer().'s','zerbscrm'); ?></h4>
   						<div class='actions'>
		        ...
		        <?php if(!zeroBSCRM_isExtensionInstalled('csvpro')){ ?>
							<a href="<?php echo admin_url('admin.php?page='.$zbs->slugs['csvlite']);?>"><div class='button import-customers'><?php _e("Import " .zeroBSCRM_getContactOrCustomer() ."s", "zero-bs-crm"); ?></div></a>
		        <?php } ;?>
   						...
   						<div class='mid-banner'>
   							<?php
							$zbstermc = sprintf( _n( '%s '.zeroBSCRM_getContactOrCustomer(), '%s '.zeroBSCRM_getContactOrCustomer().'s', $zbs_customer_total_global, 'zero-bs-crm' ), $zbs_customer_total_global);

   							?>
   							<a href='<?php echo zbsLink($zbs->slugs['customer-search']);?>'><span class='lead'><?php _e('All ' . zeroBSCRM_getContactOrCustomer() . 's');?></span><span class='sub-count'><?php echo $zbstermc; ?></span></a>
   						</div>
   						<div class='section'>
   							...
   							<?php
								...
								foreach($tags as $tag){
									...
									$zbstermc = sprintf( _n( '%s '.zeroBSCRM_getContactOrCustomer(), '%s '.zeroBSCRM_getContactOrCustomer().'s', $tag['count'], 'zero-bs-crm' ), $tag['count']);
