...
function zeroBSCRMJS_showContactLinkIf(contactID){
	...
	if (typeof contactID != "undefined" && contactID !== null && contactID !== ''){
		...
		if (contactID > 0){
				...
				var navButton = '<a target="_blank" style="margin-left:6px;" class="zbs-trans-quicknav-contact ui icon button blue mini labeled" href="' + window.zeroBSCRMJS_transactionedit_links.editcontactprefix + contactID + '"><i class="user icon"></i> ' + zeroBSCRMJS_transEditLang('contact','Contact') + '</a>';
	...
	jQuery('#zbs-customer-title .zbs-view-contact').unbind('click').click(function(){
		...
		if (typeof contactID != "undefined" && contactID !== null && contactID !== ''){
			...
			if (contactID > 0){
				...
				var url = window.zeroBSCRMJS_transactionedit_links.editcontactprefix + contactID;
				window.open(url, '_blank');
