...
class zbsDAL_transactions extends zbsDAL_ObjectLayer {
    ...
    protected $objectModel = array(
        ...
        'taxes'            => array(
                // db model:
                'fieldname' => 'zbst_taxes', 'format' => 'str',
                // output model
                'input_type' => 'tax',
                'label' => 'Taxes',
                'placeholder'=>''
        ...
        'tax'            => array('fieldname' => 'zbst_tax', 'format' => 'decimal'),
        'total'            => array(
                // db model:
            ...
            #} ============= PRE-QUERY ============
                ...
                if ($withCustomFields && !$onlyID){
                    ...
                    if (is_array($custFields)) foreach ($custFields as $cK => $cF){
                        ...
                        // add as subquery
                        $extraSelect .= ',(SELECT zbscf_objval FROM '.$ZBSCRM_t['customfields']." WHERE zbscf_objid = transaction.ID AND zbscf_objkey = %s AND zbscf_objtype = %d) '".$cK."'";
        ...
        #} ============= PRE-QUERY ============
            ...
            if ($withCustomFields){
                ...
                if (is_array($custFields)) foreach ($custFields as $cK => $cF){
                    ...
                    // add as subquery
                    $extraSelect .= ',(SELECT zbscf_objval FROM '.$ZBSCRM_t['customfields']." WHERE zbscf_objid = transaction.ID AND zbscf_objkey = %s AND zbscf_objtype = %d) ".$cKey;
    ...
    private function tidy_transaction($obj=false,$withCustomFields=false){
            ...
            */
            ...
            // status categorisation (basically did it succeed?) Used in things like invoice partials calcs
            // for now hard-typed for mvp v3. 
            // this is an issue for things like small case e.g. __("succeeded",'zero-bs-crm'); made this fail. Used in Invoice Partials.
            // it should be if status of transaction is in the array of statuses to include - then OK deduct. Otherwise return -1.
            $res['status_bool'] = -1; if ($res['status'] == __('Succeeded','zero-bs-crm') || $res['status'] == __('Completed','zero-bs-crm')) $res['status_bool'] = 1;
            ...
            $res['date_date'] = (isset($obj->zbst_date) && $obj->zbst_date > 0) ? zeroBSCRM_date_i18n(-1,$obj->zbst_date,false,true) : false;   
            ...
            $res['created_date'] = (isset($obj->zbst_created) && $obj->zbst_created > 0) ? zeroBSCRM_locale_utsToDatetime($obj->zbst_created) : false;
