...
function zeroBSCRM_locale_wpConversionWorkarounds($format='d/m/Y'){
    ...
    if ($format == 'j F, Y') $format = 'd/m/Y'; // added 2/7/18 for https://epicplugins.groovehq.com/groove_client/mailboxes/49227/filters/188769/tickets/87154663
    
        // added 27/2/19 https://zbsltd.atlassian.net/browse/ZBS-792
        if ($format == 'j. F Y') $format = 'd/m/Y'; 
        if ($format == 'j F Y') $format = 'd/m/Y'; 

        // added 11/03/19 https://epicplugins.groovehq.com/groove_client/tickets/109670521 ZBS-817
        if ($format == 'F j, Y') $format = 'm/d/Y'; 
...
//hence the 1 Jan 1970 bug reported in the date picker from Paul (https://epicplugins.groovehq.com/groove_client/mailboxes/49227/filters/188769/tickets/85858971)
...
function zeroBSCRM_date_i18n_plusTime($format,$timestamp){
    ...
    return zeroBSCRM_date_i18n($format,$timestamp);
...
//      WH: As of 16/8/18 - has extra var $isGMT - 
...
function zeroBSCRM_date_i18n($format, $timestamp, $notused=true, $isGMT=false) {

    // catch empty timestamps
    if (!is_numeric($timestamp) || $timestamp <= 0) $timestamp = time();
