        ...
        if (
            ...
            #} ============= PRE-QUERY ============
                ...
                /*if ($withCustomFields && !$onlyID){
                    ...
                    #} Retrieve any cf
                    $custFields = $this->DAL()->getActiveCustomFields(array('objtypeid'=>ZBS_TYPE_QUOTETEMPLATE));

                    #} Cycle through + build into query
                    if (is_array($custFields)) foreach ($custFields as $cK => $cF){

                        // add as subquery
                        $extraSelect .= ',(SELECT zbscf_objval FROM '.$ZBSCRM_t['customfields']." WHERE zbscf_objid = quotetemplate.ID AND zbscf_objkey = %s AND zbscf_objtype = %d) '".$cK."'";
                        
                        // add params
                        $params[] = $cK; $params[] = ZBS_TYPE_QUOTETEMPLATE;
        ...
        #} ============= PRE-QUERY ============
            ...
            /*if ($withCustomFields){
                ...
                if (is_array($custFields)) foreach ($custFields as $cK => $cF){
                    ...
                    // add as subquery
                    $extraSelect .= ',(SELECT zbscf_objval FROM '.$ZBSCRM_t['customfields']." WHERE zbscf_objid = quotetemplate.ID AND zbscf_objkey = %s AND zbscf_objtype = %d) ".$cKey;
