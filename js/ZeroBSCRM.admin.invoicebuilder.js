 ...
 * V1.0
 *
 * Copyright 2017-2019, Zero BS Software Ltd. & Zero BS CRM.com
 *
 * Date: 23rd Jan 2019
 */
...
//the getting of the invoice - using the data array returned from the AJAX call above (which is in the Zero BSCRM.Control.Invoices.php)
...
//the drawing of the HTML - using the data array returned from the AJAX call above (which is in the Zero BSCRM.Control.Invoices.php)
...
function zbscrm_JS_draw_invoice_actions_html(res){
	...
	html += '<div id="zbs_invoice_actions">';
		...
		if (!res.invoiceObj.new_invoice){
			html += '<a href="' + res.invoiceObj.preview_link + '" target="_blank" class="ui button blue" id="zbs_invoice_preview">' + zbscrm_JS_invoice_lang('preview')  +'</a>';
	...
	html += '<div class="zbs-invoice-topper">';
		html += '<table class="form-table">';
			...
			if (res.invoiceObj.settings.hideid != "1"){
            	...
            	if (typeof res.invoiceObj.id_override != "undefined" && res.invoiceObj.id_override != "") potentialInvID = res.invoiceObj.id_override;
            ...
            if (typeof res.invoiceObj.due_date != "undefined" && res.invoiceObj.due_date != null){
	...
	html += '<div id="zbs-invoice-customiser">';
		html += '<div class="ui grid" style="margin: 0em 0.5em;">';
			...
			html += '<div class="eight wide column" style="padding:0">';
				...
				// space for extension
	            if (typeof window.zbscrm_JS_draw_invoiceCustomiserExtras == 'function') html += zbscrm_JS_draw_invoiceCustomiserExtras(res);
...
function zbscrm_JS_draw_partials_table(res){
	...
	var html = '<div id="zbs-invoice-partial-payments" class = "ui grid">';
		...
		jQuery.each(res.invoiceObj.partials, function (i, v) {
			...
			html += '<div class="zbs-partial-value half ri">' + vValue + '</div>';
			html += '</div>';
...
function zbscrm_JS_generate_invoice_row(res, i, v){
	...
	rowhtml += '<input class="form-control" type="text" name = "zbsli_itemname[]" value = "' + item_title + '" placeholder="' + zbscrm_JS_invoice_lang('rowtitleplaceholder') + '" /><br/>';
	...
	rowhtml += '<input type="number" class="zbsli_quan zbsli_quan'+i+' form-control quan" data-zbsr="' + i + '" name = "zbsli_quan[]" value = "' + v.quantity + '" min="0" step="0.1">';
	rowhtml += '</td>';
...
function zbscrm_JS_bind_row_actions(){
	...
	// add empty row
	jQuery('#zbs-add-new-row').unbind('click').bind('click', function(){
	...
	jQuery(".zbs-item-block input[type=number]").bind('keyup mouseup', function () {
		...
		row_tot = quan * price;
		jQuery(this).parent().siblings('.row-amount-' + zbs_row_to_up).html(row_tot.toFixed(2));
...
function zbscrm_JS_calculate_invoice_row_subtotals(){
	...
	jQuery('.zbs-invoice-row').each(function(index, pvalue){
		...
		row_tot = quan * price;
		jQuery('.row-amount-' + zbs_row_to_up).html(row_tot.toFixed(2));
...
function zbscrm_JS_calculate_invoice_subtotal(){
	...
	jQuery('#subtotal-value').html(invoice_subtotal.toFixed(2));
		...
		for (var i = 0; i < this_row_amount.length; i++) {
			...
			if (this_row_amount[i] > 0){
				if (tax_percent[i] == 0){
					...
					var taxRate = parseFloat(taxRateLine.rate).toFixed(2);
	...
	jQuery.each(tax_table_output, function (i, v) {
		if (i > 0){
			...
			tax_html += '<div class="total-row row">';
				...
				tax_html += '<div class="tax-amt zbs-total-tax third ri">' + tax_table_output[i].toFixed(2) + '</div>';	
			tax_html += '</div>';
		}
	});
...
function zbscrm_JS_calc_grandtotal(){
		...
		if (debugTotalling) console.log('TOTAL:' + zbs_gt_invoice_tot.toFixed(2));
		if (debugTotalling) console.log('=========== / Totals ==========');

		jQuery('#zbs-inv-grand-total').html(zbs_gt_invoice_tot.toFixed(2));
		...
		// SHOULD BE REPLACED with php variant of js total calc code here, later
		// ... smt like zeroBSCRM_invoicing_calcTotal($inv);
		jQuery('#zbs-inv-grand-total-store').val(zbs_gt_invoice_tot.toFixed(2));
...
function zbscrm_JS_calc_amount_due(){
	...
	if (window.invoice_partial){
		...
		jQuery('#inv-amount-due').html(amount_due);
...
function zbscrm_JS_calculatediscount(){
	...
	if (zbs_discount_amt != ''){
		if (zbs_discount_type == 'm'){
			...
			jQuery('#discount-value').html(parseFloat(zbs_discount_amt).toFixed(2));
		} else {
			...
			jQuery('#discount-value').html(parseFloat(zbs_discount_combi).toFixed(2));
		}
	}
...
function zbscrm_JS_invoice_typeahead_bind(){
	...
	}).bind('typeahead:selected', select).bind('typeahead:change', change);
...
function zeroBSCRMJS_bindContactLinkIf(){
	...
	jQuery('#zbs-customer-title .zbs-view-contact').unbind('click').click(function(){
...
function zeroBSCRMJS_bindCompanyLinkIf(){
	...
	jQuery('#zbs-company-title .zbs-view-company').unbind('click').click(function(){
...
function zbscrm_JS_bind_invoice_actions(){
	...
	// Download
	jQuery('#zbs_invoicing_download_pdf').unbind("click").bind("click", function (e) {
		e.preventDefault();
	...
	// send email button
	jQuery('#zbs_invoicing_send_email').unbind('click').click(function(ind,ele){
