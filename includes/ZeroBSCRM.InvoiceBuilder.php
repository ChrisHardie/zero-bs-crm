    ...
    if (!empty($invoicePostID)){
        ...
        if (!empty($templatedHTML)){
                        ...
                        if($invlines != ''){
                            ...
                            foreach($invlines as $invline){
                                ...
                                '<tr>
                                            ...
                                            <td style="width:15%;text-align:center;" class="cen">'.$invline['zbsli_quan'].'</td>
            ...
            */
            ...
            $html = str_replace('###LANGINVREF###',__('Reference:','zero-bs-crm'),$html);
                            ...
                            foreach($invlines as $invline){
                                ...
                                '<tbody class="zbs-item-block" data-tableid="'.$i.'" id="tblock'.$i.'">
                                        <tr class="top-row">
                                            ...
                                            <td style="width:15%;text-align:center;" rowspan="3" class="cen">'.$invline['zbsli_quan'].'</td>
...
function zbs_invoice_pdf(){
    ...
    if (!$zbs->isDAL3()){
    	...
    	if ( isset($_POST['zbs_invoicing_download_pdf'])  ) {
            ...
            $options->set('defaultFont', 'SourceSansPro');
...
function zeroBSCRM_invoicing_generateStatementPDF($contactID=-1,$returnPDF=false){
    ...
    #} Left this as per Invoice PDF gen
    #} ... we'll auto-delete as we go, and use hashes, so not a GIANT vector... though needs solidifying.
    $upload_dir = wp_upload_dir();
    #$user_dirname = $upload_dir['basedir'].'/'.$current_user->user_login .'/invoices/';    
    #} WH changed this, might as well dump them in a central repo, but do delete them as you go... a username is not enough to hide them
    ...
    if ($contactID > 0 && $templatedHTML !== ''){
        ...
        // STATEMENT table
            ...
            $sTable = '<table border="0" cellpadding="0" cellspacing="0" class="body" style="border-collapse:collapse;mso-table-lspace:0pt;mso-table-rspace:0pt;background-color:#FFF;width:100%">';
                ...
                $sTable .= '<th cellpadding="0" cellspacing="0" >'.__('Reference','zero-bs-crm').'</th>';
...
function zeroBSCRM_invoicing_generateInvoiceHTML($invoiceID=-1,$template='pdf',$html=''){
    ...
    // number
    $invOffset = 0; // none v3.0+
    $invIDStyles = ''; // none initially
    $thisInvReference = $invoice['id'];
    if (isset($invoice['id_override']) && !empty($invoice['id_override'])) {
        $thisInvReference = $invoice['id_override'];
        // and we don't show ID:
        $invIDStyles = 'display:none;';
    }
    ...
    if ($zbsCompanyID > 0){
        ...
        if (is_array($invTo) && isset($invTo['coname'])){
    ...
    #} Mikes previous invoice.php worked into vars:
        ...
        // v3.0 really we just use ref, perhaps some of this legacy can be culled?
        $invNoStr = '';
        if (isset($allowInvNoChange) && $allowInvNoChange == '1'){ 
            if (isset($thisInvReference)) {
                $invNoStr = $thisInvReference; 
            }
         } else {
            #} Just output it, not overridable
            $invNoStr = '<div class="zbs-normal">'.$thisInvReference.'</div>';
        } 
    ...
    #} == Template -> HTML build
        ...
        // invnostr replaces this v3.0+ // $html = str_replace('###REF###',$ref,$html);  
                ...
                if ($invlines != ''){
                    ...
                    foreach($invlines as $invline){
                        ...
                        '<tr>
                                    ...
                                    <td style="width:15%;text-align:center;" class="cen">'.$invline['quantity'].'</td>
                    ...
                    foreach($invlines as $invline){
                        ...
                        '<tbody class="zbs-item-block" data-tableid="'.$i.'" id="tblock'.$i.'">
                                <tr class="top-row">
                                    ...
                                    <td style="width:15%;text-align:center;" rowspan="3" class="cen">'.$invline['quantity'].'</td>
