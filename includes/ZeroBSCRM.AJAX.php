...
/* ======================================================
   ...
   #} Set Transients for checking of the subscription status..
   add_action('wp_ajax_zbs_extension_transient', 'zbs_extension_transient');
   function zbs_extension_transient(){

   		check_ajax_referer( 'zbs-ext-trans-ajax-nonce', 'security' );  //nonce it up...

		if (zeroBSCRM_permsCustomers())	{

			//was going to use transients here to check every day, but switched to CRON instead
			//found this though: https://deliciousbrains.com/background-processing-wordpress/
			set_transient('zbs_extension_check', true, 24 * HOUR_IN_SECONDS);
			update_option('zbs_account_connected', true);
			update_option('zerobscrm_dismissed_zbs-connected-account',1);


			if(isset($_POST['items']) && !empty($_POST['items'])){
				// Sanitize? is this going to be an array?
				$items = $_POST['items'];
			}else{
				$items = 'None';
			}

			$zbs_access['email'] 	= sanitize_text_field($_POST['email']);
			$zbs_access['key'] 		= sanitize_text_field($_POST['key']);

			//store the access data for future checks against server..  WH input how best here..
			update_option('zbs_account_access', $zbs_access);
			update_option('zbs_account_items', $items);
...
function ZeroBSCRM_send_quote_email(){
    ...
    $contactEmail = ''; if (isset($_POST['em'])) $contactEmail =  sanitize_text_field($_POST['em']);
    $contactID = -1; if (isset($_POST['cid'])) $contactID =  (int)sanitize_text_field($_POST['cid']);


	//validate the email
	if (!zeroBSCRM_validateEmail($contactEmail)){
	...
	#} Check id + perms + em
	if ($quoteID == -1 || empty($contactEmail)){
		die();
	}
	...
	$body = zeroBSCRM_mailTracking_addPixel($body,-12, $customerID, $customerEmail, $quoteID);

    $form = zeroBSCRM_mailTemplate_get(ZBSEMAIL_NEWQUOTE);
    ...
    if($active){
		...
		zeroBSCRM_mailTracking_logEmail(ZBSEMAIL_NEWQUOTE, $customerID, -12, $customerEmail, $quoteID);
	...
	*/
	...
	if ($active){
		...
		// Attachments?
        $attachments = array();
        $zbsSendAttachments = get_post_meta($quoteID, 'zbs_quote_sendattachments', true);
        if ($zbsSendAttachments == "1"){
        	$quoteFiles = get_post_meta($quoteID, 'zbs_customer_quotes', true);
        	if (is_array($quoteFiles) && count($quoteFiles) > 0){
        		...
        		foreach($quoteFiles as $quoteFile){
                    ...
                    $filename = basename($quoteFile['file']);
                    ...
                    $attachments[] = array($quoteFile['file'],$filename);
	...
	//sends the notification via wp_mail (for now)... 
...
function ZeroBSCRM_accept_quote(){
	...
	#} I think this one might be OK to not have a perms check as we are checking for a logged in user.
	#} The action is not a nopriv action, so they need to be logged in and any user can be given a quote to accept
	#} v3.0 brings magic hashes too as extra security here but no specific permissions are needed to accept a quote
	#} apart from beig logged in, which is handled by thie being only hooked to wp_ajax_ and not wp_ajax_nopriv
    ...
    $zbsQuoteData = zeroBS_getQuote($quoteID,true);
    	...
    	if ($zbs->isDAL3()){
    		...
    		// dal3
    		if (isset($zbsQuoteData) && is_array($zbsQuoteData)) {
    			
    			$okayToProceed = true;
    			$correctHash = $zbsQuoteData['hash'];
    ...
    if ($okayToProceed){
		...
		#} Check email... (though allow through either way, as is quote accepting..)
		#} in fact, no point
		#} zeroBSCRM_validateEmail($quoteSignedBy)

        if ($correctHash === $quoteHash) {

			#} An actual quote exists with this id...
			...
			if ($correctHash === $quoteHash) {
        		...
        		if ($zbs->isDAL3())
        			$quoteCreatorEmail = zeroBS_getObjOwnerWPEmail($quoteID,'zerobs_quote');
        		else
        			// old cpt way (DAL 1+2)
        			$quoteCreatorEmail = zeroBS_getCreatorEmail($quoteID);


					#} Update quote as accepted (should verify this worked...)
					zeroBS_markQuoteAccepted($quoteID,$quoteSignedBy);

					#} Send notification to creator of quote
					$quoteCreatorEmail = zeroBS_getCreatorEmail($quoteID);
					...
					if (!empty($quoteCreatorEmail)){
						...
						$subject = __('Proposal Accepted',"zero-bs-crm");
				...
				#} If it gets here, count as success. 
				header('Content-Type: application/json');
...
====================================================== */
	...
	function zbs_lead_form_views(){
		...
		$zbs_form_id = (int)sanitize_text_field($_POST['id']);   //only passing a form ID (which is (int) set and then updating a counter) should be secure (WH INPUT)
		$zbs_form_views = get_post_meta($zbs_form_id,'zbs_form_views',true);
		if($zbs_form_views == ''){
			$zbs_form_views = 1;
		}else{
			$zbs_form_views++;
	...
	function zbs_lead_form_capture() {
	    ...
	    if($zbs_honey != ''){
	    	...
	    	//we have done the inserting... is not AJAX (but might want to change this to an AJAX submit(?))... in fact.. DO IT.
	    	#$r['message'] = 'Contact inserted as a LEAD';
	    	#} less obv msg:
	...
	function zeroBSCRM_AJAX_mailDelivery_validateSMTPPorts(){
		...
		if (!empty($smtpPort)){
			...
			if (!$remotePortCheck[0]){
				$res['debugs'][] = __('Zero BS CRM cannot connect to '.$smtpHost.' on port: '.$smtpPort.', this may not matter, as ZBS will try other ports for you below.','zero-bs-crm'); 
				$okay = false;
			} 		
				...
				$attemptedSend = zeroBSCRM_mailDelivery_checkSMTPDetails($sendFromName,$sendFromEmail,$smtpHost,$smtpPort,$smtpUser,$smtpPass);
					...
					if (is_array($attemptedSend) && isset($attemptedSend['sent']) && $attemptedSend['sent']){
						...
						if (isset($attemptedSend['finset']) && is_array($attemptedSend['finset'])){
								...
								// old ver if (!function_exists('phpe_enc')) {  require(ZEROBSCRM_PATH . 'includes/ZeroBSCRM.Encryption.php'); }
								if (!function_exists('zeroBSCRM_encrpytion_unsafe_enckey')) {  require(ZEROBSCRM_PATH . 'includes/ZeroBSCRM.Encryption.php'); }

									#} Encrypt PW:
									$encryptionKey = zeroBSCRM_getSetting('smtpkey');
									// old ver
									//if (empty($encryptionKey)) {$encryptionKey = phpe_key(); $zbs->settings->update('smtpkey',$encryptionKey);}
									//$encPW = phpe_enc($smtpPass, $encryptionKey);
									//if (empty($encryptionKey)) {$encryptionKey = zeroBSCRM_encrpytion_unsafe_enckey(); $zbs->settings->update('smtpkey',$encryptionKey);}
									//$encPW = zeroBSCRM_encrpytion_unsafe_enc($smtpPass, $encryptionKey, true);
									$encPW = zeroBSCRM_encrpytion_unsafe_process('encrypt',$smtpPass,$encryptionKey,$encryptionKey);
		...
		if (!empty($sendFromName)){
			...
			if (!empty($sendFromEmail) && zeroBSCRM_validateEmail($sendFromEmail)){
				...
				// checks out, send a test :)
					...
					$subject = '[Zero BS CRM] Mail Delivery Routine';
					$headers = array('Content-Type: text/html; charset=UTF-8');	
									...
									if (in_array('company',$columnsRequired)){
							            ...
							            if (!empty($coID)){
							            	...
							            	if (!isset($companyNameCache[$coID])){
								                ...
								                if (empty($coName)) $coName = zeroBSCRM_getCompanyOrOrg().' #'.$co['id'];
			...
			/* ==============================================================================
			 ===================== COMPANY =============================================== */
				...
				case 'company':
					...
					if (in_array('transactiontotal', $columnsRequired) || in_array('transactioncount', $columnsRequired)){
					...
					$companies = zeroBS_getCompaniesv2(true,$per_page,$page_number,$possibleSearchTerm,$argsOverride,$possibleTagIDs,$inArray,$withTags,$withAssigned,$latestLog,$sortField,$sortOrder,$possibleQuickFilters,$withTransactions);
...
/* ======================================================
	Admin AJAX: Telemetry
====================================================== */

	#} Telemetry end of period (calls home)
	function zeroBSCRM_teleEndPeriod(){
	...
	/*
		...
		if ($share == "1"){  //only share if permission given
			...
			#} Keep log of tele's
		  	$runCount = get_option('zbs_telecount',0);
		  	$wizCount = get_option('zbs_wizard_run',0);
			...
			if (is_callable('curl_init')){ 
				...
				#} Page loads, usr count, deac/act
				$activePeriod = get_option('zbs_teleactive');
				$age = -1; if (is_array($activePeriod) && isset($activePeriod['tstart']) && $activePeriod['tstart'] > 0) $age = floor((time()-$activePeriod['tstart'])/86400);
		  		$usrs = get_option('zbs_teleactiveusrs',array());
				$theme = wp_get_theme();
				$tD = array('n'=>'unknown'); if ( $theme->exists() ){
					$tD = array('n' => $theme->get( 'Name' ),'v' => $theme->get('Version'));
				} # / theme
				// use zeroBSCRM_allPluginListSimple for ALL plugin
				$pL = zeroBSCRM_activePluginListSimple(); # / plugin list // https://wordpress.stackexchange.com/questions/52144/what-wordpress-api-function-lists-active-inactive-plugins
				$url = home_url();

				$m = array('tmm'=>md5(time().$url),'ta'=>$activePeriod,'age'=>$age,'u'=>$url,'t'=>$tD,'p'=>$pL,'wc'=>$wizCount,'pc'=>$runCount,'uc'=>count($usrs));


			    $postData = http_build_query($m);

			  	$ch = curl_init();
			  	curl_setopt($ch, CURLOPT_URL,  $zbs->urls['tmm'] );
			    curl_setopt( $ch, CURLOPT_POST, true );
			    curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
			    curl_setopt( $ch, CURLOPT_POSTFIELDS, $postData);
			    $x = curl_exec( $ch );
			    ...
			    #} clear telemetry if return success
			    if ($x == 'fini') {
			    	update_option('zbs_teleactive', array('tstart'=>time()));
					update_option('zbs_teleactiveusrs', array());
					
					// this stores a log of any 'failed to send'
					// not implmented yet: delete_option('zbs_telewalls');
				}

				#} Keep count of reruns
		  		update_option('zbs_telecount', $runCount+1);
...
function zeroBSCRM_AJAX_addTag(){
	...
	if(zeroBSCRM_permsIsZBSUserOrAdmin()){
		...
		/* Actually, don't need to switch here, just switch the objtype param :) then will work for all types
		switch ($objType){

			case 'contact':
				// addtag to contact (WILL BE DAL2)
				...
				if (!empty($tagID)) {
					...
					// here we lie re: slug, just because is only for 1 page load + saves reget
					$slug = $zbs->DAL->makeSlug($objTag);
...
====================================================== */
	...
	function zeroBSCRM_AJAX_betaFeedback(){
		...
		// retrieve deets
		$comm = ''; if (isset($_POST['comm']) && !empty($_POST['comm'])) $comm = zeroBSCRM_textProcess($_POST['comm']);
		$email = ''; if (isset($_POST['email']) && !empty($_POST['email'])) $email = sanitize_email($_POST['email']);
		$page = ''; if (isset($_POST['page']) && !empty($_POST['page'])) $page = sanitize_text_field($_POST['page']);
		$area = ''; if (isset($_POST['area']) && !empty($_POST['area'])) $area = sanitize_text_field($_POST['area']);
...
====================================================== */
	...
	function zeroBSCRM_sendJSONError($errObj=''){
