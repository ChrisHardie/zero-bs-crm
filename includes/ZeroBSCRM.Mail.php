...
function zeroBSCRM_mailDelivery_sendViaSMTP($smtpHost='',$smtpPort='',$smtpUser='',$smtpPass='',$smtpSecurity='tls',$sendFrom='',$sendFromName='',$toEmail='',$toName='',$subject='',$msgText='',$msgHTML='',$debugMail=false,$returnDebug=false,$attachments=false){ #,$ccEmails='',$bccEmails='',$replyToAddr='',$returnPath=''
	...
	if (empty($sendFromName)) $sendFromName = 'Zero BS CRM';
	...
	!empty($smtpPort)
		...
		global $zbsDebug; $zbsDebug['return'] = $returnDebug; $zbsDebug['debug'] = $debugMail;
			...
			set_error_handler(function($errno, $errstr, $errfile, $errline, array $errcontext) { 
					...
					if ($zbsDebug['debug']){
						...
						if ($zbsDebug['return'])
							$retDebugStr .= '===<br />PHP Level Warning '.$errno.': '.$errstr.'<br />';
						else
							echo 'PHP Level Warning '.$errno.': '.$errstr.'<br />';
		...
		// https://epicplugins.groovehq.com/groove_client/mailboxes/49227/filters/188769/tickets/94129371
		// since 2.95.4 (21/9/18)
			...
			//$phpmailer->SMTPDebug = 2;
				...
				if ($phpmailer->Send()) {
					...
					if ($debugMail) {
				        ...
				        /* debug 
						$retDebugStr .= '<hr /><strong>Mail FAILED #exit-2</strong><br />';
						$retDebugStr .= '<hr />RAMEY:'.print_r(array(true,$phpmailer->Host,$phpmailer->Username,$phpmailer->Password),1).'<br><hr>';
						$retDebugStr .= '<hr />RAMEY2:'.$e->getCode().'</hr>'; */
    			...
    			// general exception
					...
					if ($debugMail) {
						...
						if (!$returnDebug) {
							...
							return $retDebugStr.'===<br />Final Error<br />error:'.$e->getMessage();
						}
					}
					return false;
    		} finally {
	...
	if (
			...
			if (
				...
				if (
					...
					$emTextBody = '';
					$emSubject = '[Zero BS CRM] '.__('Mail Delivery Routine',"zero-bs-crm");
...
function zeroBSCRM_mailDelivery_retKeyData($str=''){
	if (!function_exists('zeroBSCRM_encrpytion_unsafe_process')) {  require(ZEROBSCRM_PATH . 'includes/ZeroBSCRM.Encryption.php'); }

              #} Unencrypt PW:
              $key = zeroBSCRM_getSetting('smtpkey');
              $unenc = zeroBSCRM_encrpytion_unsafe_process('decrypt',$str,$key,$key);
              return $unenc;
}
