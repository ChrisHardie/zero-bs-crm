...
function zeroBSCRM_isCustomerPortalDisabled($cID=-1){
		...
		#} No Contact ID, no entry, unless Admin or Zero BS Admin we can let those in.
		$cID = (int)$cID;
		if ($cID < 1) {
	...
	if (!empty($cID) && !empty($email) && zeroBSCRM_validateEmail($email)){
		...
		if ( null == email_exists( $email )) {
			...
			if ($user->exists()){
				...
				// add role(s)
				$user->set_role( 'zerobs_customer' );   //lets create a Zero BS Customer Role...
   				...
   				// assign company from slave -> master
   					...
   					if (empty($masterCompany)){
   						...
   						if (!empty($slaveCompany)){
   							...
   							$changes[] = __('Assigned main record to secondary record\'s company',"zero-bs-crm").' (#'.$slaveCompany.').';
   						...
   						if (!empty($slaveCompany) && $slaveCompany != $masterCompany){
							...
							// conflicting change
			        		$conflictingChanges[] = __('Secondary contact was assigned to company, whereas main record was assigned to another company.',"zero-bs-crm").' (#'.$slaveCompany.').';
   ...
   // if returnAsInt - will return -1 for not published, -2 for not accepted, or 14int timestamp for accepted
	...
	function zeroBSCRM_setMaxQuoteID($newMax=0){
		...
		if ($newMax >= $existingMax){
			...
			//DAL1+2: return (int)update_option('quoteindx',$newMax);
   ...
   function zeroBS___________InvoiceHelpers(){return;}
	...
	function zeroBSCRM_setMaxInvoiceID($newMax=0){
		...
		if ($newMax >= $existingMax){
			...
			#} harsh override! doesn't even check! :o
			//DAL1+2: return (int)update_option('invoiceindx',$newMax);
	...
	function zeroBSCRM_get_invoice_settings(){
	    ...
	    $invoice_settings = array(
	        ...
	        'lang'                  => array(
	                ...
	                'add_edit'          => zeroBSCRM_slashOut(__('Edit Company Details', 'zero-bs-crm'),true),
	                ...
	                'company' => zeroBSCRM_slashOut(__('Company', 'zero-bs-crm'),true), // contact view button (if assigned)
	                'view' => zeroBSCRM_slashOut(__('View', 'zero-bs-crm'),true),
	                'addnewcontact' => zeroBSCRM_slashOut(__('Add New Contact', 'zero-bs-crm'),true),
	                'newcompany' => zeroBSCRM_slashOut(__('New Company', 'zero-bs-crm'),true),
	 ...
	 *                  invoice_address_to:    // 0 contact or 1 company. So if assigned to Mike, can be address to a company (i.e. Mike Stott: Zero BS CRM, Mike Stott: Epic Plugins) etc
	 * 
	 * 
...
/* ======================================================
   ...
   function zeroBS___________TransactionHelpers(){return;}
	...
	function zeroBS_tranCount($status=false){
  	...
  	// wh migrated to DAL3, 'presumed' params :)
	function zeroBS_getTransactionsRange($ago=-1, $period='days'){
		...
		if ($utsFrom > 0){
			...
			return $zbs->DAL->transactions->getTransactions(array(
				'newerThan' => $utsFrom,
	            'sortByField'   => 'zbst_created',
            	'sortOrder'     => 'ASC',
				'page' => -1 // just copying from below :) (all of them - DANGEROUS)
				));
...
/* ======================================================
   ...
   function zeroBS___________SettingsHelpers(){return;}
	...
	function zeroBSCRM_getCompanyOrOrg(){
	    ...
	    $theStr = __('Company',"zero-bs-crm"); 
	    if ($coOrOrg == 'org') $theStr = __('Organisation',"zero-bs-crm");
		if ($coOrOrg == 'domain') $theStr = __('Domain',"zero-bs-crm"); # WH 5/1/18 - added for Janos - long term, think about
	    return $theStr;

	}

	// gets company label (plural)
	function zeroBSCRM_getCompanyOrOrgPlural(){

		#} Curr
	    $coOrOrg = zeroBSCRM_getSetting('coororg');
	    $theStr = __('Companies',"zero-bs-crm"); 
	    if ($coOrOrg == 'org') $theStr = __('Organisations',"zero-bs-crm");
	    if ($coOrOrg == 'domain') $theStr = __('Domains',"zero-bs-crm"); # WH 5/1/18 - added for Janos - long term, think about
...
/* ======================================================
   ...
   ====================================================== */
	...
	function zeroBSCRM_storeAPIKey($api_key=''){
		...
		// DAL 3 uses our settings model :) update_option('zbs_crm_api_key', $api_key);
		global $zbs; 
	...
	function zeroBSCRM_updateAPIKey($api_key=''){
		...
		//... is basically a wrapper for this
	...
	function zeroBSCRM_storeAPISecret($api_secret=''){
		// DAL3 migrated: update_option('zbs_crm_api_secret', $api_secret);
		global $zbs; 
		return $zbs->DAL->updateSetting('api_secret',$api_secret);
	}
...
/* ======================================================
   ...
   ====================================================== */
	...
	function zeroBS_getObjOwnerWPMobile($objID =-1,$objType='zerobs_customer'){
		...
		if ($objID > 0){
		    ...
		    $ownerID = zeroBS_getOwner($cID,false,$objType);
   ...
   #} WP POSTS! - deprecated! we don't store original creator against our DB2 objs, so DO NOT USE, use zeroBS_getObjOwnerWPEmail
   function zeroBS_getCreatorEmail($postID=-1){
