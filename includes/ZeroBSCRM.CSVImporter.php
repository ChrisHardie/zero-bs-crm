          ...
          <h2 class="zbs-white"><span class="add-new-button"><?php _e('CSV Importer Lite',"zero-bs-crm"); if (!empty($subpage)) echo ': '.$subpage; ?></span>
            ...
            <div class="ui special popup top left transition hidden" id="learn-pop">
              ...
              <div class="content-left">
                <p>
                    <?php _e("If you have contacts you need to import to Zero BS CRM, doing so via a CSV is a quick and easy way to get your data in.","zero-bs-crm"); ?>
                </p>
                <p>
				...
				##WLREMOVE
				?>
                <a href="https://zerobscrm.com/kb/knowledge-base/what-should-my-csv-be-formatted-like/" target="_blank" class="ui button orange"><?php _e("Learn More","zero-bs-crm");?></a>
				<?php
				##/WLREMOVE
				?>
			  </div>
              <div class="video">
                  <p>
                  		...
                  		<img src="<?php echo ZEROBSCRM_URL ?>i/learn/learn-csv-import.png" />
...
function zeroBSCRM_CSVImporterLitehtml_app(){
	...
	$settings = array('savecopy'=>false,'defaultcustomerstatus'=>'Customer'); #} LITE JUST GETS HARD TYPED
					...
					<div class="zbscrm-import-log">
						...
						<?php 
								...
								if ($lineIndx == 0 && $ignoreFirstLine){
									...
									#} split
	   								...
	   								if (count($customerFields) > 0){
	   									...
	   									#} Already exists? (This is only used to find dupes, temp fix for https://epicplugins.groovehq.com/groove_client/mailboxes/49227/filters/188770/tickets/64566165 )
	   									...
	   									# DEBUG echo 'output:<pre>'.print_r(array($userUniqueID,$customerFields,$newCustID),1).'</pre>';

	   									#echo '<pre>'; print_r(array($userUniqueID,$customerFields)); echo '</pre>';
	   									if (!empty($newCustID) && empty($potentialCustomerID)) {
											...
											if (!empty($potentialCustomerID)){
			   									...
			   									#} Line
												echo '<div class="zbscrm-import-log-line">Customer Already Exists!: #'.$newCustID.'... <i class="fa fa-user"></i><span>[Updated]</span></div>';
							...
							if (count($existingOverwrites) > 0) {
								...
								echo '<div class="zbscrm-import-log-line"><strong>The following customers were already in your Zero BS CRM, and were updated:</strong></div>';
		...
		default: #} Also case 0
			...
			?><div class="zbscrm-csvimport-wrap">
				...
				<div class="zbscrm-upload-csv">
					<p class="zbscrm-csv-import-help"><?php _e("If you have a CSV file of customers that you would like to import into ZBS, you can start the import wizard by uploading your .CSV file here.","zero-bs-crm");?></p>
					<form method="post" enctype="multipart/form-data" class="zbscrm-csv-import-form">
