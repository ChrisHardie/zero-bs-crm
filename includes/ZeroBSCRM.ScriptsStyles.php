...
function zeroBSCRM_scriptStyles_enqueueJSRoot(){
    ...
    $zbs_root = array(
    	'crmname' => __('Zero BS CRM','zero-bs-crm'),
        'root' => ZEROBSCRM_URL,
	...
	//
	// the 2nd one is what showed up with https://zbsltd.atlassian.net/browse/ZBS-384
...
function zeroBSCRM_calendar_admin_styles(){
    ...
    // WH Note: No idea, but SOME references on this page would produce zbs//css instead of zbs/css - so using untrailingslashit
    wp_enqueue_style(  'select2', untrailingslashit(ZEROBSCRM_URL) .'/css/select2.min.css');
    wp_enqueue_script(  'select2js', untrailingslashit(ZEROBSCRM_URL) . '/js/lib/select2.min.js');
	...
	wp_register_script('zerobscrm-events-js' , ZEROBSCRM_URL .'js/ZeroBSCRM.admin.task.min.js', array('jquery','wh-moment-v2-8-1-js','zerobscrm-calendar-js','select2js'));
...
/* 3.0 moved this into zeroBSCRM_calendar_admin_styles where it makes sense (centralised)
function zeroBSCRM_add_admin_calendar_scripts($hook) {

	global $post;

	 if (isset($post) && $post->post_type == 'zerobs_event') {
...
add_action( 'admin_enqueue_scripts', 'zeroBSCRM_add_admin_calendar_scripts');
*/
